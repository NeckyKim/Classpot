...
export default function CheckApplicantCode() {
  ...
  function findApplicantCode(event: any) {
    ...
    onSnapshot(
      query(
        collection(dbService, 'users', userCode, 'tests', testCode, 'applicants'),
        where('shortApplicantCode', '==', shortCode),
      ),
      (snapshot) => {
        setResults(snapshot.docs.map((current) => current.id));
      },
    );
  }
  ...
  return (
    <div className={styles.background}>
      <div className={styles.container}>
        ...
        <SubmitButton text="응시 코드 확인" onClick={findApplicantCode} disabled={shortCode.length !== 6} />
      </div>
    </div>
  );
}
