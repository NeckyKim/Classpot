...
export default function CheckTestCode() {
  ...
  function findUserCode() {
    onSnapshot(
      query(collection(dbService, 'users'), where('shortUserCode', '==', shortCode.substring(0, 4))),
      (snapshot) => {
  ...
  function findTestCode() {
    if (userCode) {
      onSnapshot(
        query(
          collection(dbService, 'users', userCode, 'tests'),
          where('shortTestCode', '==', shortCode.substring(4, 8)),
        ),
        (snapshot) => {
  ...
  return (
    <div className={styles.background}>
      <div className={styles.container}>
        ...
        <SubmitButton text="응시 코드 확인" onClick={findTestCode} disabled={shortCode.length !== 8} />
      </div>
    </div>
  );
}
