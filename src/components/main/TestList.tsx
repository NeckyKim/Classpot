...
import GetTestList from '../utils/GetTestLists';
import GenerateShortTestCode from '../utils/GenerateShortTestCode';
  ...
  return (
    <div className={styles.background}>
      <div className={styles.container}>
        <div className={styles.header}>시험 목록</div>
          ...
          <Link to={'/test/' + elem.testCode} style={{ textDecoration: 'none' }}>
            <div className={styles.testElements}>
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, rgba(${colorPalette[elem.color][0]}, ${
                    colorPalette[elem.color][1]
                  }, ${colorPalette[elem.color][2]}) 20%, rgb(${colorPalette[elem.color][0] - 40}, ${
                    colorPalette[elem.color][1] - 40
                  }, ${colorPalette[elem.color][2] - 40}) 80%)`,
                }}
              />

              <div className={styles.testName}>{elem.testName}</div>
            </div>
          </Link>
        ))}
        ...
        {isCreatingTest && (
          <Modal title="시험 추가" onClose={() => setIsCreatingTest(false)}>
            <div className={styles.colorButtonContainer}>
              {colorPalette.map((elem: any, index: number) => (
                <div
                  ...
                  style={{
                    background: `linear-gradient(135deg, rgba(${colorPalette[index][0]}, ${colorPalette[index][1]}, ${
                      colorPalette[index][2]
                    }) 20%, rgb(${colorPalette[index][0] - 40}, ${colorPalette[index][1] - 40}, ${
                      colorPalette[index][2] - 40
                    }) 80%)`,
                  }}
            ...
            <InputBox type="text" value={testName} onChange={(event: any) => setTestName(event.target.value)} />
            <br />
            <br />
            <br />

            <Buttons>
              <SubmitButton text="추가" onClick={createTest} disabled={testName.replace(/(\s*)/g, '') === ''} />
            </Buttons>
          </Modal>
        )}
      </div>
