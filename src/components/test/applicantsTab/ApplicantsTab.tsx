...
export default function ApplicantsTab({
  ...
  function downloadApplicantList() {
    ...
    applicantList.map((elem: any) => {
      ...
      ws['!cols'] = [{ wpx: 120 }, { wpx: 240 }, { wpx: 720 }];
      return false;
    });
      ...
      <div className={styles.container}>
        ...
        <div className={styles.containerTop}>
          <div className={styles.info}>
            <Label style={{ marginBottom: 0 }}>총 응시자 수</Label>

            <div className={styles.infoValue}>{applicantList.length} / 30명</div>
          </div>
          ...
          <SubmitButton text="응시자 목록 다운로드" onClick={downloadApplicantList} />
        </div>
        ...
        {applicantList.length > 0 ? (
          <div>
            ...
            {applicantList.map((elem: any) => (
              <ApplicantContainer userCode={userCode} testCode={testCode} applicantObject={elem} />
            ))}
          </div>
        ) : (
          <div className={styles.empty}>
            ...
            <div className={styles.emptyMainText}>응시자가 없습니다.</div>

            <div className={styles.emptySubText}>오른쪽 상단의 [응시자 추가] 버튼을 눌러서 응시자를 추가해주세요.</div>
          </div>
        )}

        {isAddingApplicant && (
          <AddApplicant userCode={userCode} testCode={testCode} setIsAddingApplicant={setIsAddingApplicant} />
        )}
      </div>
    </div>
  );
