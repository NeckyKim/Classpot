...
export default function ApplicantContainer({
  userCode,
  testCode,
  applicantObject,
}: {
  ...
  async function editApplicant() {
    if (userCode && testCode) {
      try {
        await updateDoc(
          doc(dbService, 'users', userCode, 'tests', testCode, 'applicants', applicantObject.applicantCode),
          {
  ...
  return (
    <div>
      <div className={styles.container}>
        ...
        <div className={styles.applicantCode}>{applicantObject.shortApplicantCode}</div>

        <div className={styles.applicantName}>{applicantObject.applicantName}</div>

        <div className={styles.applicantScore}>{sumArray(applicantObject.reportCard)}점</div>

        <div
          className={applicantObject.pause ? styles.applicantPauseOffButton : styles.applicantPauseOnButton}
          onClick={() => {
        ...
        <img
          ...
          onClick={() => {
            navigator.clipboard.writeText(
              `${window.location.origin}/apply/manager/${userCode}/test/${testCode}/applicant/${applicantObject.applicantCode}`,
            );
      ...
      {isEditingApplicant && (
        <Modal title="응시자 수정" onClose={() => setIsEditingApplicant(false)}>
          ...
          <InputBox type="text" value={applicantName} onChange={(event: any) => setApplicantName(event.target.value)} />
          <br />
          <br />
          <br />

          <Buttons>
            <SubmitButton text="수정" onClick={editApplicant} disabled={applicantName.replace(/(\s*)/g, '') === ''} />

            <CancelButton text="취소" onClick={() => setIsEditingApplicant(false)} />
          </Buttons>
        </Modal>
      )}
      ...
      {isDeletingApplicant && (
        <Modal title="응시자 삭제" onClose={() => setIsDeletingApplicant(false)}>
          ...
          <br />
          <Buttons>
            <DeleteButton text="삭제" onClick={deleteApplicant} disabled={applicantName.replace(/(\s*)/g, '') === ''} />

            <CancelButton text="취소" onClick={() => setIsDeletingApplicant(false)} />
          </Buttons>
        </Modal>
      )}
    </div>
