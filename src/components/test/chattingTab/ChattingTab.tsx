  ...
  async function sendChatting(event: any) {
    ...
    if (userCode && testCode && applicantIndex !== null && applicantList[applicantIndex]?.applicantCode) {
      ...
      localStorage.setItem(
        `Chatting_${userCode}_${testCode}_${applicantList[applicantIndex]?.applicantCode}`,
        String(currentTime),
      );
      ...
      try {
        ...
        await updateDoc(
          doc(
            ...
            testCode,
            'applicants',
            applicantList[applicantIndex]?.applicantCode,
          ),
          {
            chatting: [
              ...applicantList[applicantIndex].chatting,
              {
                sender: 'manager',
                time: currentTime,
                text: chattingText,
              ...
              <div>
                <div className={styles.applicantHeader}>
                  ...
                  <div className={styles.applicantLastTime}>
                    {localStorage.getItem(`Chatting_${userCode}_${testCode}_${elem.applicantCode}`) ===
                    String(elem.chatting.slice(-1)[0]?.time)
                      ? elem.chatting.length > 0 &&
                        (new Date(Date.now()).toLocaleDateString() ===
                        new Date(elem.chatting.slice(-1)[0]?.time).toLocaleDateString()
                          ? new Date(elem.chatting.slice(-1)[0]?.time).toLocaleTimeString('en-US', {
                              hour: 'numeric',
                              minute: 'numeric',
                            })
                          : new Date(elem.chatting.slice(-1)[0]?.time).toLocaleDateString().slice(0, -1))
                      : elem.chatting.length > 0 && <div className={styles.applicantUnread} />}
                  </div>
                </div>

                <div className={styles.applicantLastMessage}>{elem.chatting.slice(-1)[0]?.text}</div>
              </div>
            </div>
          ))
        ) : (
          ...
          >
              ...
              {applicantList[applicantIndex]?.chatting.length > 0 ? (
                applicantList[applicantIndex]?.chatting?.map((elem: any, index: number) => (
                  <div>
                    ...
                    {(index === 0 ||
                      (index > 0 && applicantList[applicantIndex]?.chatting[index - 1]?.sender !== elem.sender) ||
                      (index > 0 &&
                        new Date(applicantList[applicantIndex]?.chatting[index - 1]?.time).toLocaleDateString() !==
                          new Date(elem.time).toLocaleDateString())) && (
                      <div className={elem.sender === 'manager' ? styles.senderManager : styles.senderApplicant}>
                        ...
                        <div className={styles.senderTime}>
                          {new Date(applicantList[applicantIndex]?.chatting[index]?.time).toLocaleTimeString('en-US', {
                            hour: 'numeric',
                            minute: 'numeric',
                          })}
                        </div>
            ...
            <div className={styles.chattingInputBoxZone}>
              <input
                ...
                spellCheck={false}
                placeholder={
                  isTestTime.isTime === 'running'
                    ? '이곳에 내용을 입력하세요.'
                    : '채팅 기능은 시험 진행 중일 때만 사용할 수 있습니다.'
                }
                disabled={isTestTime.isTime !== 'running'}
              />

              <button type="submit" value="전송" className={styles.sendButton} disabled={!chattingText}>
                <img src={process.env.PUBLIC_URL + '/icons/dashboard/send.svg'} />
              </button>
            </div>
          </form>
        ) : (
          <div className={styles.notSelected}>응시자를 선택해주세요.</div>
        )}
      </div>
    </div>
  );
