    ...
    <div className={styles.container}>
          ...
          {applicantList.map((elem: any, index: number) => (
            <div className={styles.applicantElements}>
              <div className={styles.applicantInfo}>
                ...
                {!elem.finished ? (
                  elem.submitted !== 0 ? (
                    <div className={styles.statusRunning}>진행중</div>
                  ) : (
                    <div className={styles.statusBefore}>이전</div>
                  )
                ) : (
                  <div className={styles.statusFinished}>완료</div>
                )}
              </div>

              <div>
                <div className={styles.infoLabel}>제출 시간</div>
              ...
              <div>
                ...
                <div className={styles.infoValue}>0점</div>
              </div>
      ...
      {showLog && (
        <Modal title="시험 행동 로그" onClose={() => setShowLog(false)}>
          ...
          <div className={styles.logApplicantName}>{applicantList[applicantIndex]?.applicantName}</div>
          ...
          {Object.values(applicantList[applicantIndex]?.log).map((elem: any, index: number) => (
            <div className={styles.logElements}>
              ...
              <div className={styles.logTime}>{new Date(elem.time).toLocaleTimeString()}</div>

              <div className={styles.logAction}>{elem.action}</div>
