        ...
        for (let j = 0; j < questionList?.length; j++) {
            ...
            if (questionList[j]?.type === "mc" && applicantList[i]?.answerSheet[j] !== null) {
                ...
                if (questionList[j]?.grading === 0) {
                    // 정답
                    if (JSON.stringify(questionList[j].answer) === JSON.stringify(Object.values(applicantList[i]?.answerSheet[j]))) {
                        reportCard[j] = questionList[j].points;
                    }

                    // 미응답
                    else if (JSON.stringify(new Array(questionList[j].choices.length).fill(false)) === JSON.stringify(Object.values(applicantList[i]?.answerSheet[j]))) {
                        reportCard[j] = null;
                    }
                ...
                // 채점: 응답 시 만점
                    ...
                    // 응답
                    if (JSON.stringify(new Array(questionList[j].choices.length).fill(false)) !== JSON.stringify(Object.values(applicantList[i]?.answerSheet[j])) && applicantList[i]?.answerSheet[j] !== null) {
                        reportCard[j] = questionList[j].points;
                    }
                            ...
                            <div className={styles.infoContainer}>
                                ...
                                <div>
                                    ...
                                    <div className={styles.infoValue}>
                                        ...
                                        <div className={styles.infoValue2}>
                                            ({(correct / questionList.length * 100).toFixed(1)}%)
                                        </div>
                                    </div>
                                </div>
                            </div>
